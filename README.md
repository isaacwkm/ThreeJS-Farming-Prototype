...
## Localization
12/8/2024 - In its current state, the game supports English, Chinese as our logographic script, and Arabic as our right-to-left script. A dropdown menu is now located on the window for players to choose specific languages. The handling for this menu is set within its own module which also exports the functionality for localization to any objects that display text in game. As the person writing this was not responsible for the localization, they do not know how the specific process went for localizing each language, and had requested that someone more familiar take care of this portion of the devlog, but alas.
...
## Reflection
For the mobile installation of this assignment, our deployment setup on Github Actions didn't preserve the directory structure of our project, making it difficult to use the service worker and web manifest effectively. As such, a workaround was needed in order to test everything locally, which was achieved by using USB Developer Mode on Android. In regards to team contributions to the project, there is certainly room for improvement in regards to individual time management and overall work distribution. It has become increasingly apparent this past week that there needs to be more general emphasis on collaborative programming and group awareness of the underlying design goals. As a final note, we'll take the remaining time we have to try and focus on player feedback as there woefully hasn't been enough attention paid to that.
